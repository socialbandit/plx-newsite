...
// CUSTOM_OpenViaCurl() added 09.07.2011_rg
...
function CUSTOM_OpenViaCurl($local_url){ // Customization made 09.07.2011_rg because fopen() is not permitted to oopen url's (based on the server config.)
	$local_result = '';
	// Open connection
	$ch = curl_init();
	// Set the parameters
...
function event_makesitemap() {
	...
	foreach($urls as $url) { // Loop over <url> elements in xml file
		...
		if (!stristr($loc, 'sitemap')) { // Web page is not sitemap
			...
			$filecontent = CUSTOM_OpenViaCurl($loc);
